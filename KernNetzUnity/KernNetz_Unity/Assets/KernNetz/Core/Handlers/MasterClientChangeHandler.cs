...
namespace KernNetz.Handlers
{
    internal class MasterClientChangeHandler : IHandler
    {
        ...
        public void HandleMessage(Message message, uint PeerId)
        {
            ...
            FigNet.KernNetz.KN_Internals.IsMasterClient = (bool)operation.Parameters[1];
            // todo: here set is kinematic to false if entity is not locked
            FigNet.KernNetz.KN.OnMasterClientUpdate?.Invoke(FigNet.KernNetz.KN_Internals.IsMasterClient);
            FN.Logger.Info($"NEW MASTER IN TOWN Client Status of {playerId} is {FigNet.KernNetz.KN.IsMasterClient} | {(bool)operation.Parameters[1]}");
        }
    }
}
