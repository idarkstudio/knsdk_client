...
namespace KernNetz
{
    public abstract class NetworkEntity
    {
        ...
        {
            if (KN.Socket.IsConnected && entityType != EntityType.Player)
            {
                ...
                KN.Socket.SendMessage(instantiateOp, DeliveryMethod.Reliable);
            }
        }
        ...
        public static void Delete(EntityType entityType, uint networkId)
        {
            if (KN.Socket.IsConnected && entityType != EntityType.Player)
            {
                ...
                KN.Socket.SendMessage(deleteOp, DeliveryMethod.Reliable);
            }
        }
        ...
        public bool BindNetProperty<T>(byte index, T property, DeliveryMethod deliveryMethod, Action<T> callBack, bool invokeForOwner = false, SyncType syncType = SyncType.OnValueChange) where T : IFNProperty
        {
            bool result = false;
            if (IsMine)
            {
                property = default(T);
                result = SetNetProperty<T>(index, property, deliveryMethod, syncType);
            }
            else
            {
        ...
        public void GetNetProperty<T>(byte index, DeliveryMethod deliveryMethod, Action<T> callBack) where T : class, IFNProperty
        {
            if (!States.ContainsKey(deliveryMethod))
            {
                States.Add(deliveryMethod, new EntangleState() { DeliveryMethod = (int)deliveryMethod });
            ...
            var type = GetPropertyType<T>(default);
        ...
        public void Send(EntangleState delta)
        {
            if (KN.Socket.IsConnected)
            {
                ...
                KN.Socket.SendMessage(op, (DeliveryMethod)delta.DeliveryMethod, channelID);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void ApplyStateDelta(DeliveryMethod method,EntangleState delta)
        {
            if (!IsMine)
            {
                try
                {
                    States[method].ApplyStateDelta(delta);
                }
                catch (Exception)
                {
                    if (!States.ContainsKey(method))
                    {
                        States.Add(method, new EntangleState() { DeliveryMethod = (int)method });
                        States[method].ApplyStateDelta(delta);
                    }
                }
            }
